...
public:
    using EventEnumType = tcp_events;
    ...
    tcp_ppr(bool is_client, std::function<void(bool)> close_fn)
...
public:
    generated_events<EventEnumType> handle_packet_send(rte_packet& pkt){
        ...
        ge.event_happen(tcp_events::pkt_in);
        if(_t.armed()) {
            _t.cancel();
            _t.arm(std::chrono::seconds(async_flow_config::flow_expire_seconds));
        }
    ...
    struct async_flow_config {
        ...
        static constexpr int flow_expire_seconds = 15;
